/* ═══════════════════════════════════════════════════════════
   NEXORA ACADEMY — CERTIFICATE RENDERER (self-contained)
   ═══════════════════════════════════════════════════════════ */
...
function certRenderHTML(cert){
  ...
  <div class="certificate" id="certificateNode">
    ...
    <div class="cert-content">
        ...
        <div class="cert-logo-wrap">
          <div class="cert-logo-badge">
            <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 30 L50 20 L80 30 L80 72 L50 62 L20 72 Z" fill="#0B1F3A" stroke="#D4A63A" stroke-width="3" stroke-linejoin="round"/>
      ...
      <div class="cert-completion-text">has successfully completed all the academic and graduation requirements for the course</div>
      <div class="cert-course-name">${certEsc(cert.course_name || 'Course')}</div>

      <div class="cert-body">
        <div class="cert-meta">
          <div class="cert-meta-item"><div class="cert-meta-icon"><i class="fas fa-calendar-alt"></i></div><div class="cert-meta-text"><div class="cert-meta-label">Course Duration</div><div class="cert-meta-value">${certEsc(cert.course_duration || 'Self-paced')}</div></div></div>
          <div class="cert-meta-item"><div class="cert-meta-icon"><i class="fas fa-calendar-check"></i></div><div class="cert-meta-text"><div class="cert-meta-label">Date Issued</div><div class="cert-meta-value">${certFmtDate(cert.issued_date)}</div></div></div>
          <div class="cert-meta-item"><div class="cert-meta-icon"><i class="fas fa-award"></i></div><div class="cert-meta-text"><div class="cert-meta-label">Grade</div><div class="cert-meta-value">${certEsc(cert.grade || 'Competent')}</div></div></div>
          <div class="cert-meta-item"><div class="cert-meta-icon"><i class="fas fa-user"></i></div><div class="cert-meta-text"><div class="cert-meta-label">Student ID</div><div class="cert-meta-value">${certEsc(cert.student_id || '—')}</div></div></div>
        </div>
        ...
        <div class="cert-qr-block">
          ...
          <div class="cert-qr-box"><img src="${certQrUrl(verifyUrl)}" alt="QR code to verify certificate"></div>
          <div class="cert-qr-caption">Scan QR Code to Verify<br>or visit:</div>
          <div class="cert-qr-url">nexoraacademy.com/verify</div>
        </div>
      </div>

      <div class="cert-closing">In recognition of your dedication, hard work, and successful completion of all requirements, this certificate is proudly awarded to you.</div>
      ...
      <div class="cert-footer-note">This certificate is issued electronically and is valid without a signature.</div>
    </div>
  </div>`;
}
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="cert-toolbar" style="margin-top:24px">
        <button class="cert-btn cert-btn-primary" onclick="window.print()"><i class="fas fa-download"></i> Download / Print</button>
        <button class="cert-btn cert-btn-outline" onclick="navigator.clipboard.writeText('${certEsc(cert.certificate_id)}').then(()=>alert('Certificate ID copied!'))"><i class="fas fa-copy"></i> Copy ID</button>
      </div>`;
  } catch (err){
    container.innerHTML = `<div class="cert-error"><i class="fas fa-exclamation-triangle"></i><h2>Certificate not found</h2><p>${certEsc(err.message || 'The certificate ID is invalid or has been revoked.')}</p><a href="/" class="cert-btn cert-btn-primary">Go to Nexora Academy</a></div>`;
  }
}
...
async function openCertificateModal(certId){
  ...
  if (!modal){
    ...
    modal.innerHTML = `
      <div class="modal-box" style="max-width:1100px;padding:20px;position:relative">
        <button onclick="closeCertificateModal()" style="position:absolute;top:12px;right:16px;background:transparent;border:none;font-size:1.8rem;cursor:pointer;color:#64748B;z-index:10">&times;</button>
        <div id="certificateModalBody"></div>
      </div>`;
    document.body.appendChild(modal);
    // ensure styles
    modal.style.cssText = 'display:none;position:fixed;inset:0;background:rgba(11,31,58,0.7);backdrop-filter:blur(6px);z-index:2000;align-items:flex-start;justify-content:center;padding:1rem;overflow-y:auto';
  }
