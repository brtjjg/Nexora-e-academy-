    ...
    try {
        ...
        try {
            ...
            for (const adm of admins.rows) {
                await notify(client, adm.id, 'assignment_submitted', 'New assignment submitted', `${req.user.full_name} submitted ${assignment.title}`, `/admin/assignments/${req.params.id}`);
            }
        } catch (e) {}
...
router.post('/submissions/:id/return', requireAdmin, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        try {
            await notify(client, sub.student_id, 'assignment_returned', 'Assignment returned', `${sub.assignment_title}: ${reason}`, `/student/assignments/${sub.assignment_id}`);
        } catch (e) {}
        await client.query('COMMIT');
        res.json({ ok: true });
    } catch (err) { await client.query('ROLLBACK'); throw err; }
